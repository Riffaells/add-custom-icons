...
│   └── ui/
│       ├── SettingsTab.ts    # Интерфейс настроек (API 1.13 + legacy display())
│       └── settings/         # Декларативные определения настроек (Obsidian 1.13+)
...
- **Расположение иконок**: папка плагина, папка хранилища (`.obsidian/icons`) или свой путь
- **Монохромные цвета**: цвета, которые заменяются на `currentColor`, чтобы иконки следовали теме
...
- **Режим отладки**: Подробные логи в консоли разработчика

На Obsidian 1.13+ вкладка собирается декларативно (`getSettingDefinitions()`), поэтому настройки
плагина попадают в глобальный поиск настроек; на более старых версиях отрисовывается прежний
интерфейс `display()` — без изменений.
